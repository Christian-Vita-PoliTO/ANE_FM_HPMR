...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.40]{images/cases_5_6_7_temperature_distributions.png}  % Modifica scale o usa width se serve
...
\begin{table}[h!]
\FloatBarrier
\caption{Uncertainty budget of the interpolated results. $\sigma_{k}$ refers to the multiplication factor, $\sigma_{\tilde{F}}$ to the fission source, averaged over the ninety cells. The two contributions are combined in quadrature before rounding, and for the fission source the combination is performed cell by cell and then averaged, so the tabulated combined values may differ by one unit in the last digit from the quadrature of the rounded entries.}
\centering
\begin{tabular}{lcccccc}
\hline
 & $\sigma_{k}$ reference & $\sigma_{k}$ database & $\sigma_{k}$ combined & $\sigma_{\tilde{F}}$ reference & $\sigma_{\tilde{F}}$ database & $\sigma_{\tilde{F}}$ combined \\
 & (pcm) & (pcm) & (pcm) & (\%) & (\%) & (\%) \\ \hline
